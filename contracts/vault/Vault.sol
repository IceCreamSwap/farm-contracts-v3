...
contract Vault is Ownable, Pausable {
    ...
    IFarm public immutable masterchef;
    ...
    function harvest() external notContract whenNotPaused {
        IFarm(masterchef).leaveStaking(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IFarm(masterchef).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IFarm(masterchef).pendingCake(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IFarm(masterchef).leaveStaking(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IFarm(masterchef).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IFarm(masterchef).enterStaking(bal);
        }
    }
